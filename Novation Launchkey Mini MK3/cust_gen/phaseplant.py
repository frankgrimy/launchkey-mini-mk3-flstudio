...
def Knobs(midiId, data1, data2):
    ...
    if plugins.isValid(channumber) and plugins.getPluginName (channumber, -1) == "Phase Plant":
            ...
            if midiId == midi.MIDI_CONTROLCHANGE:
                if not var.SHIFT_STATUS:
                    for x in knobs.knobs.values():
                        if data1 == x:
                            plugins.setParamValue(math2.linnormalize(data2, 127, 1, 0), x-14, channumber, -1)
                            handled = True
                            ui.setHintMsg("(" + plugins.getPluginName(channumber, -1, 1) + ") " + "Controlling Macro " + str(x-20) + " (" + plugins.getParamName(x-14, channumber, -1) + ")")
